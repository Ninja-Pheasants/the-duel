...
app.use(cookieParser('this is a very long hash. Please forget it! ok? :E'));
app.use(session({
	key: 'the-duel',
...
var pg = new pg.Client(POSTGRES.CONNECTION_STRING);
...
app.use(function (req, res, next) { //on each request coming to our server...
	...
	pg.connect(function (err, client, done) {
		if (err) {
			console.log("err pg.connect", err);
			next();
			return;
		}
...
app.get('/open', authMw, login, function (req, res) {
	DUEL_IS_OPEN = true;
	res.redirect("/");
});

app.get('/close', authMw, login, function (req, res) {
	DUEL_IS_OPEN = false;
	res.redirect("/");
});

app.post('/voto-giurato', authMw, login, function (req, res) {
	if (req.body.candidato && req.body.voto) {
		...
		if (req.body.messaggio) {
			...
			}, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_TOKEN_SECRET, function (err, data, resp) {
				//hoping no error!
			});
		}
	}
...
app.get('/reset', authMw, login, function (req, res) {
	VOTI = {
		aperto: false,
		giuria: {
			a: 0,
...
app.get('/login', authMw, login, function (req, res) {
	res.redirect('/');
});
...
function authMw(req, res, next) {
	...
	if (!auth || auth[0] !== (process.env.THEDUEL_USER || 'user') || auth[1] !== (process.env.THEDUEL_PASS || 'pass')) {
			// any of the tests failed
			// send an Basic Auth request (HTTP Code: 401 Unauthorized)
			res.statusCode = 401;
			// MyRealmName can be changed to anything, will be prompted to the user
			res.setHeader('WWW-Authenticate', 'Basic realm="The Duel"');
